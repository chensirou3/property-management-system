...
docker compose build --pull --no-cache mysql api web
docker compose up -d
docker compose ps
```
...
G10 看板配置与访客模拟核销主链路可独立验证：

```powershell
npx playwright test e2e/final-capabilities-lifecycle.spec.ts --workers=1
```

用例按项目和角色完成组件新增、修改、排序、预览和发布，并检查版本冲突；随后验证未脱敏访客被拒绝、脱敏访客登记、IoT 模拟进入/离开、重放、状态和 `productionConnected=false`。页面 38/49 的视觉请求使用确定性夹具，真实数据库状态机与审计由本用例和 `FinalCapabilityIntegrationTest` 覆盖。
...
制品进入忽略的 `.artifacts`。安全候选必须以 `--pull --no-cache` 构建；API/Web 的 OpenSSL 低于 `3.5.8-r0` 时构建失败。镜像扫描固定检查 API、Web、加固版 MySQL 8.4 和 Redis 的 Critical/High，先刷新 OS/Java 两套漏洞库，再用冻结的数据库快照按本地 image ID 扫描并生成 schema v3 `trivy/summary.json`；验证器会依据 SHA-256 重读原始报告并重算镜像、平台、扫描目标、包清单和漏洞计数，任何 Critical/High、空或不完整扫描结果、证据篡改或过期数据库都会失败，不使用 ignore/allowlist。CI 没有 push 步骤，其报告不能冒充生产 manifest 证据；生产候选推送后必须按 digest 和显式 `PMS_TARGET_PLATFORM` 拉取、复扫，并用 `scripts/validate-production-deployment.ps1` 在部署前及部署后核对 manifest digest、目标平台、本地 image ID、运行容器 PID 1 非 root 与证据。执行生产验证前还必须将 Secret Manager 生成的六个非空文件放入仓库外受控目录并设置绝对路径 `PMS_SECRET_DIR`；Linux 目录必须为 `0700`、文件必须为只读 `0444`，以兼容 Compose bind mount 与不同容器 UID，Windows 则使用禁继承的显式主体 allowlist 和只读文件 ACL/属性。部署验证不可跳过文件内容和权限检查；Linux CI 会用独立临时 project 实际启动四个服务并验证健康、Redis 命名空间 ACL、三网隔离、API 无默认出口与非 root Secret 可读性。恢复脚本只使用随机临时库；空库脚本只使用带精确 project 标签的隔离容器/网络/卷，并在验证 `23|91|22|5|0|4|3|0`（迁移、表、报表、策略、生产策略、已发布组件、访客、生产访客连接）、API readiness 和 Web 200 后自动清理。

生产 Compose 还要求四项服务统一设置 `PMS_TARGET_PLATFORM`。在目标 Linux Docker 主机上运行前，可按 `scripts/smoke-production-compose-linux.sh` 的逻辑读取 `docker version` 的 server OS/architecture，并规范化导出 `linux/amd64`、`linux/arm64` 或 `linux/386`；不得复制开发机平台值。正式部署的 `PMS_WEB_PORT` 必须保持 `5174`，与宿主 TLS Nginx upstream 一致；其他值只允许隔离 smoke 使用，不能通过生产验证器。验证器还会拒绝 `privileged`、`cap_add`、宿主 namespace/设备、未核准 bind（尤其 Docker socket）及通过 named-volume `driver_opts` 伪装的 bind，并在部署后复核容器 `.HostConfig` 与 `.Mounts`。Redis 生产 ACL 只允许 `pms3:*` key 和显式缓存命令白名单；除 Spring 健康检查必需的只读 `INFO` 外，不开放管理/危险类别，Pub/Sub 禁用。Linux smoke 会同时验证四个实际运行镜像的平台、允许的 `PING`/命名空间读写删，以及越界 key、`FLUSHALL`、`CONFIG`、`PUBLISH` 均被拒绝。
...
- G10 恢复检查至少包括：Flyway 为 V23、空库 91 张表、22 个 ACTIVE 报表、5 个适配器策略且 production_ready=0、4 个已发布看板组件、3 条脱敏访客种子且 production_connected=0；`system_setup` 状态须与账号存在性一致；主项目有效房屋/车位/客户/客户资产关系为 359/250/403/403、隔离项目具备 1/1/1/2/1 的网格/楼栋/单元/资产/客户链；费用定义/标准/分配为 23/17/773（其中仪表分配 30），收据号段 2、合成折扣策略 1、公摊规则版本 1；迁移/档案/费用/财务/计量既有差异均为 0。还需核对无 QUEUED/RUNNING/FAILED 报表或打印任务、任务/制品/事件/打印快照/通知正文 SHA-256 合法、打印项数/收据次数/通知计数一致、真实通知通道为 0；集成回调/尝试 SHA-256 异常为 0、开放死信和待排空模拟 Outbox 为 0，业务 Outbox 不纳入模拟排空。
...
- G1 固定视觉门禁使用 `npm run test:visual`，覆盖 1366×768、1440×900、1920×1080 的登录页与看板；G2—G10 使用 `test:catalog-smoke` 和 `test:catalog-visual` 覆盖 49 页。视觉测试把看板、迁移批次、应收任务、G6 财务、G7 计量、G8 报表/任务、G9 集成治理，以及 G10 看板配置/访客查询固定为确定性合成响应，防止依法保留的审计、账务、计量和任务证据导致截图漂移；真实 API 生命周期仍由专用 E2E 和后端集成测试覆盖。原因、人工检查和摘要见 `visual-baselines.md`。
