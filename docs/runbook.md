...
- G8 恢复检查至少包括：Flyway 为 V18、空库 84 张表、22 个 ACTIVE 报表定义；主项目有效房屋/车位/客户/客户资产关系为 359/250/403/403、隔离项目具备 1/1/1/2/1 的网格/楼栋/单元/资产/客户链；费用定义/标准/分配为 23/17/773（其中仪表分配 30），收据号段 2、合成折扣策略 1、公摊规则版本 1；迁移/档案/费用/财务/计量既有差异均为 0，另核对无 QUEUED/RUNNING/FAILED 报表或打印任务、任务/制品/事件/打印快照/通知正文 SHA-256 合法、打印项数/收据次数/通知计数一致、真实通知通道为 0。
...
- G1 固定视觉门禁使用 `npm run test:visual`，覆盖 1366×768、1440×900、1920×1080 的登录页与看板；G2—G8 使用 `test:catalog-smoke` 和 `test:catalog-visual` 覆盖 49 页。视觉测试把看板、迁移批次、应收任务、G6 财务、G7 计量和 G8 报表/任务查询固定为确定性合成响应，防止依法保留的审计、账务、计量和任务证据导致截图漂移；真实 API 生命周期仍由专用 E2E 和后端集成测试覆盖。原因、人工检查和摘要见 `visual-baselines.md`。
