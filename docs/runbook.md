...
制品进入忽略的 `.artifacts`。镜像扫描固定检查 API、Web、加固版 MySQL 8.4 和 Redis 的 Critical/High；恢复脚本只使用随机临时库；空库脚本只使用带精确 project 标签的隔离容器/网络/卷，并在验证 `20|90|22|5|0|4|3|0`（迁移、表、报表、策略、生产策略、已发布组件、访客、生产访客连接）、API readiness 和 Web 200 后自动清理。
...
- G10 恢复检查至少包括：Flyway 为 V20、空库 90 张表、22 个 ACTIVE 报表、5 个适配器策略且 production_ready=0、4 个已发布看板组件、3 条脱敏访客种子且 production_connected=0；主项目有效房屋/车位/客户/客户资产关系为 359/250/403/403、隔离项目具备 1/1/1/2/1 的网格/楼栋/单元/资产/客户链；费用定义/标准/分配为 23/17/773（其中仪表分配 30），收据号段 2、合成折扣策略 1、公摊规则版本 1；迁移/档案/费用/财务/计量既有差异均为 0。还需核对无 QUEUED/RUNNING/FAILED 报表或打印任务、任务/制品/事件/打印快照/通知正文 SHA-256 合法、打印项数/收据次数/通知计数一致、真实通知通道为 0；集成回调/尝试 SHA-256 异常为 0、开放死信和待排空模拟 Outbox 为 0，业务 Outbox 不纳入模拟排空。
