...
> 正式交付更新：V24 在全新、未初始化且种子指纹完整的数据库中清除研发种子。下文的固定合成计数用于历史 QA 回归；当前交付安装默认没有业务记录。

## 全局约定

- 主键使用 UUID 字符串；历史 QA 种子使用可识别的固定 UUID，正式交付业务记录在首次配置或数据导入时生成。
...
| 看板与访客 | `dashboard_widget_configuration`、`visitor_record` | 按项目/角色发布的预定义看板组件；全程脱敏且只允许 IoT 模拟器的访客登记、进出状态与审计证据 |
...
39. 看板组件只允许预定义指标编码，配置按 `(community_id, role_id, widget_code)` 唯一；修改、排序和发布均校验项目/角色范围、乐观锁版本和审计，发布不会接受任意 SQL 或脚本。
40. 访客姓名与手机号只能以掩码格式保存，`source_mode` 固定为 `IOT_SIMULATOR` 且 `production_connected=false`；登记、进入、离开遵循受限状态机并使用请求键/请求哈希安全重放，运行时还会复核适配器策略并 fail-closed。
...
| 已发布看板组件 | 4（预定义指标，项目与角色范围） |
| 访客记录 | 3（全部脱敏、IoT 模拟器、生产连接 0） |

全新数据库验收结果：23 个 Flyway 迁移成功、91 张表、65 个权限、22 个 ACTIVE 报表、5 个 fail-closed 适配器策略和 2 个合成基线项目；`system_setup` 记录一次性单项目首启状态。未初始化时保留两项目测试基线，网页首启后只启用用户配置的当前项目。主项目由 API/数据库动态计算为 359 套有效房屋，隔离项目为 1 套，主项目另有 250 个有效车位、403 个有效客户和 403 条有效客户资产关系。G4 的 32 条合格 + 1 条错误样本可重复完成隔离、审批、31 条生产写入 + 1 条项目映射、9 项对账和 41 条变更逆序回滚。G5/G7 合成种子为 23 个费用定义、17 个标准、773 条分配，其中 `743` 条 `ASSET` 分配、`30` 条 `METER` 分配；孤儿检查按 `target_type` 分别关联资产或仪表，两个项目均为 0。G6 增加 2 个收据号段和 1 个合成折扣策略；七项财务对账差异为 0。G7 增加 1 个公摊规则版本和计量读数/IoT/换表/计费证据，COUNT/USAGE/AMOUNT 三项对账差异为 0。G8 固定 22 个报表定义；G9 固定四个模拟器和一个禁用适配器，生产就绪为 0；G10 固定 4 个已发布看板组件和 3 条脱敏访客记录，访客生产连接数为 0。客户资产孤儿关系、跨项目关系、重复有效关系和非法面积均为 0。开发运行库允许保留已停用、已回滚、已冲正或由生命周期追加的 E2E 历史证据，因此阶段对账以空库种子、有效状态和不可变账本守恒值为准。
