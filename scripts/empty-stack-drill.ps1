...
try {
    ...
    Set-ProcessEnvironment 'PMS_BOOTSTRAP_ADMIN_USERNAME' 'empty-drill-admin'
    Set-ProcessEnvironment 'PMS_BOOTSTRAP_ADMIN_PASSWORD' (New-StrongLocalSecret)
    ...
    $webResponse = Invoke-WebRequest -Uri "http://127.0.0.1:$WebPort/login" -TimeoutSec 15 -UseBasicParsing
    if ($webResponse.StatusCode -ne 200) { throw 'The isolated web login page is not reachable.' }
    ...
    if ($started -and -not $KeepStack) {
        $labels = & docker ps -a --filter "label=com.docker.compose.project=$projectName" --format '{{.Label "com.docker.compose.project"}}'
        if (@($labels | Where-Object { $_ -and $_ -ne $projectName }).Count -gt 0) {
