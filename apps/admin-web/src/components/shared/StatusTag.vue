...
const type = computed(() => {
  if (['ACTIVE', 'NORMAL', 'SUCCESS', 'SUCCEEDED', 'COMPLETED', 'PAID', 'true'].includes(code.value)) return 'success'
  if (['FAILED', 'ERROR', 'CANCELLED', 'INACTIVE', 'DISABLED', 'false'].includes(code.value)) return 'danger'
  if (['PENDING', 'QUEUED', 'RUNNING', 'PROCESSING', 'PARTIAL', 'PARTIAL_FAILED', 'PARTIAL_PAID'].includes(code.value)) return 'warning'
  return 'info'
})
</script>
