...
├─ 启动PMS3.cmd                 Windows 双击启动入口
├─ 打开PMS3网页.url             服务运行后直接打开管理端
├─ start-pms3.ps1              自动生成技术密钥并启动四个服务
...
这是完整 Web 系统，不是可脱离 API 和数据库单独运行的静态 `index.html`。首次使用或服务停止时应双击 `启动PMS3.cmd`；服务已经运行时可双击 `打开PMS3网页.url` 直接进入管理端。
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\start-pms3.ps1
```
...
- 从全新目录双击 `启动PMS3.cmd`，确认自动生成技术密钥并启动四个服务；
