...
版本：`v0.1.0-formal-delivery.2-one-click`
...
脚本第一次运行时会自动生成 `.env`，其中只保存数据库、Redis、JWT 和回调验签所需的随机技术密钥；脚本不会创建业务账号，也不会在屏幕上显示这些密钥。随后脚本检查 Linux 容器和 Compose v2、构建并启动四个服务，健康检查通过后自动打开 <http://localhost:5174>。
